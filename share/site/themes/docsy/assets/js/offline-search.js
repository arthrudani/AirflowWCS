...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            url: $searchInput.data('offline-search-index-json-src'),

            beforeSend: function(xhr){
                xhr.overrideMimeType("application/html; charset=utf-8");
            }
        }).then(
            (data) => {
                idx = lunr(function () {
        ...
        const render = ($targetSearchInput) => {
            ...
            if (results.length === 0) {
                ...
                results.forEach((r) => {
                    ...
                    $entry.append(
                        // Removing the last part of the filepath ()file name) as it is repeated from the folder name
                        $('<small>').addClass('d-block text-muted').text(r.ref.replace(/\/([^\/?]+)\/$/g, ''))
                    );
