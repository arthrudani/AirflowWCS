...
public class ScalePort extends PortController
{
	//
  ...
  private static final int NO_ACTIVITY_TIME = 50000;//1*60*1000; // 1 min
  private int mnKeepAliveInterval = NO_ACTIVITY_TIME;
  private int mnKeepAliveTimeOut = 10000;
  private int mnReceivedKeepAliveInterval = NO_ACTIVITY_TIME + 30000;
  private ReceivedKeepAliveTimeout mpreceivedKeepAliveTimeout;
  private String msKeepAliveResponse = "ES";
  NoActivityTimeout mpnoActivityTimeout;
  ...
  protected void processReceivedDataBlock()
  {
    if (verifyReceivedData())
    {
      ...
      if(!receivedDataString.equals(msKeepAliveResponse))
      {
        publishEquipmentEvent(receivedDataString, 0);
      }
    }
    setupReceiveCycle();
    //Cancel the port time out timer
  ...
  public void initialize(String aControllerKeyName)
  {
    ...
    logger.logDebug("ScalePort.initialize()");
  }
  ...
  public void shutdown()
  {
  	...
  	super.shutdown();
    logger.logDebug("ScalePort -- shutdown() -- Start");
    logger.logDebug("ScalePort -- shutdown() -- End");
  }
  
  @Override
  protected void startupProtocol()
  {
    logger.logDebug("ScalePort.startupProtocol() - Start");
    ScaleMessage vpMessage = Factory.create(ScaleMessage.class);
  	transmitEquipmentData(vpMessage.getBufferClearCommand());
    logger.logDebug("ScalePort.startupProtocol() - End");
  }
  ...
  {
    synchronized(ScalePort.this)
    {
      transmitEquipmentData(new String(mabkeepAlive));
      if (mnKeepAliveInterval > 0)
      {
        timers.setSSTimerEvent(mpnoActivityTimeout, mnKeepAliveInterval);
        timers.setSSTimerEvent(mpreceivedKeepAliveTimeout, mnKeepAliveTimeOut);
      }
   }
  }
  /*--------------------------------------------------------------------------*/
  ...
  void receivedKeepAliveTimeout_run()
  {
    synchronized(ScalePort.this)
    {
      logger.logOperation(LogConsts.OPR_DEVICE, "ReceivedKeepAlive Timeout (" +
                mnReceivedKeepAliveInterval + " msec) - Shutting Down Port");
      String s = "ReceivedKeepAliveTimeout (" + mnReceivedKeepAliveInterval + " msec) - Shutting Down ComPort";
  ...
  {
    return Factory.create(ScalePort.class);
  }
