...
public class RackButton extends PolygonButton
{
  private Rectangle mpRackEnd1;
  private Rectangle mpRackEnd2;
   ...
   * @param ipPermissions
   */
  public RackButton(EquipmentMonitorFrame ipEMF, int inPanelIndex,
      ...
      SKDCScreenPermissions ipPermissions)
  {
    ...
    // Racks will always be Daifuku's light purple
    mpStatusColor = DAIFUKU_LIGHT_PURPLE;
    
    // Figure out where we will label the rack
    if (mpPolygon.getBounds().height > mpPolygon.getBounds().width)
    {
      mpRackEnd1 = mpPolygon.getBounds();
      mpRackEnd1.height = Math.min(mpRackEnd1.width, mnDefaultFontSize+10);
      
      mpRackEnd2 = mpPolygon.getBounds();
      mpRackEnd2.y = mpRackEnd2.y + mpRackEnd2.height - mpRackEnd1.height;
      mpRackEnd2.height = mpRackEnd1.height;
    }
    else
    {
      mpRackEnd1 = mpPolygon.getBounds();
      mpRackEnd1.width = mpRackEnd1.height;
      
      mpRackEnd2 = mpPolygon.getBounds();
      mpRackEnd2.x = mpRackEnd2.x + mpRackEnd2.width - mpRackEnd2.height;
      mpRackEnd2.width = mpRackEnd2.height;
    }
  }
  ...
  {
    if (msStatusText2 == null)
    {
      super.drawText(g, msStatusText, msStatusText2);
    }
    else
    {
      g.setColor(getTextColor(BORDER_COLOR));
      drawTextInRectangle(g, mpRackEnd1, msStatusText);
      drawTextInRectangle(g, mpRackEnd2, msStatusText);
    }
  }
}
