 ...
 *         PLC Item Arrived (27) Message related methods, if its common for all
 *         the PLC message
 *
 */
public class ItemArrivedMessage extends StandardInboundMessage {
...
//			if (loadData == null) {
//				result = false;
//				logger.logError("No Load Data found for Item Arrived message, [Load Id]" + getLoadId());
//				throw new DBException("Load Record for Load " + getLoadId() + " not found!");
//			} else
			if (stdata == null) {
				result = false;
				logger.logError("Invalid Station Id from Item Arrived message, [Station Id]" + getStationId());
				throw new DBException("Station Record for Station " + getStationId() + " not found!");
			}
		} catch (DBException ex) {
			logger.logError("Error while validating the PLC Item Arrived Message, Error" + ex.getMessage());
			return result;
		}
		return result;
	}
	...
	public boolean parse(String sMsg) {
		logger.logDebug("Parse PLC Item Arrived  Message : " + sMsg);
		if (!StringUtil.isNullOrEmpty(sMsg)) {
			...
			if (splitedMsg != null) { // && splitedMsg.length == PLCMessages.PLC_ITEM_RELEAZED_MSG_BODY_LEN
				...
				setStationId(String.format("%010d", Integer.parseInt(splitedMsg[6])));// Lane ID / Station ID

				logger.logDebug("PLC Item Arrived message Parsed sucessfully");
				return true;
			} else {
				logger.logDebug("Invalid PLC Item Arrived message from PLC for parsing");
				return false;
			}
		} else {
			logger.logDebug("PLC Item Arrived message from PLC for parsing Parse is Empty");
			return false;
		}
	}
	...
	public void processAck() {
		...
		itemAckMsg.setMessageType(PLCConstants.PLC_ITEM_ARRIVED_ACK_MSG_TYPE);
	...
	public EBSTransactionContext process() {
		ItemArrivedContext plcItemArrivedContext = null;
		try {
			findAndSetStation();
			plcItemArrivedContext = new ItemArrivedContext(stationData, getLoadData(), getLoadLineData(),
					getStationId(), getOrderId(), getLoadId(), getGlogalId(), getLineId(), getSerialNumber(),
					getDeviceId());
			Factory.create(StandardItemArrivedTransaction.class).execute(plcItemArrivedContext);

		} catch (DBException ex) {
			logger.logException(ex);
		}
		return plcItemArrivedContext;
	}
	...
	void findAndSetStation() {
		try {
			...
			logger.logError("Error while validationg the PLC Item Arrived Message, Error" + ex.getMessage());
		}
	...
	public void outputTransactionLog(EBSTransactionContext ebsTransactionContext) {
		ItemArrivedContext plcItemArrivedContext = (ItemArrivedContext) ebsTransactionContext;
		mpEBSLoadServer.logLoadItemArrivedTransaction(plcItemArrivedContext.getLoadData(),
