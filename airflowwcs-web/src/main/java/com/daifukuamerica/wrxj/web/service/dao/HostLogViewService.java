...
import com.daifukuoc.wrxj.custom.ebs.dbadapter.data.WrxHostLog;
 ...
 * @author mandrus
 */
public class HostLogViewService
{
	private static final String metaId = "WRxHostLog";
	...
	{
		return listSearch(Factory.create(WrxHostLogData.class));
	}
	...
	public TableDataModel listSearch(WrxHostLogData historyKey) throws DBException, NoSuchFieldException
	{
		if (historyKey.getOrderByColumns().length == 0)
		{
			historyKey.addOrderByColumn(WrxHostLogData.DATE_TIME_NAME, true);
		}
		List<Map> tableData = Factory.create(WrxHostLog.class).getDataQueueMessagesForWeb(historyKey);
